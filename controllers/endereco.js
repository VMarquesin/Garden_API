...
module.exports = {
   async listarEndereco(request, response) {
      try {
         //instruções SQL
         const sql = ` SELECT
            end_id , end_cep , end_bairro , end_rua , bai_id 
            FROM endereco`;
   ...
   async cadastrarEndereco(request, response) {
      try {
         ...
         const { end_cep, end_bairro, end_rua, bai_id } = request.body;
         //instrução SQL
         const sql = `INSERT INTO endereco
            (end_cep , end_bairro , end_rua , bai_id)
            VALUES (?, ?, ?, ?, ?)`;
         //definiçaõ dos dados a serem inseriodos em um array
         const values = [end_cep, end_bairro, end_rua, bai_id];
   ...
   async editarEndereco(request, response) {
      try {
         ...
         const sql = `UPDATE endereco SET end_cep  = ?,
          end_bairro  = ?, end_rua  = ?, bai_id  = ?
          WHERE end_id  = ?;`;
         //preparo do array com dados que serão atualizados
         const values = [end_cep, end_bairro, end_rua, bai_id, end_id];
